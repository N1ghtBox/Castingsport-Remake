...
const contestScoreValidator = (multipleOf: number) => (params: GridPreProcessEditCellProps<number, Contestant & { isNew: boolean }>) => {
    ...
    if (params.props.value % multipleOf !== 0) {
        return { ...params.props, error: `Wartość musi być wielokrotnością ${multipleOf}` };
    }

    if (params.props.value < 0 || params.props.value > 100)
        return { ...params.props, error: "Wartość musi być w przedziale od 0 do 100" };
